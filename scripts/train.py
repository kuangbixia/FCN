...
class Trainer(object):
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            # 每10次迭代计算一次平均loss
            if iteration % 10 == 0:
                train_writer.add_scalar('loss/{}'.format(self.args.model),
                                        running_loss / 10, iteration)
                # logger.info('running loss:{:.4f}\n'.format(running_loss / 10))
                running_loss = 0.0
            ...
            if not self.args.skip_val and iteration % val_per_iters == 0:
                total_pixAcc, total_mIoU = self.validation()
                train_writer.add_scalar('metric/pixAcc_{}'.format(self.args.model),
                                        total_pixAcc / len(self.val_loader), iteration)
                train_writer.add_scalar('metric/mIoU_{}'.format(self.args.model),
                                        total_mIoU / len(self.val_loader), iteration)
                # logger.info('pixel accuracy:{:.4f}, mIoU:{:.4f}\n'.format(total_pixAcc / len(self.val_loader),
                # total_mIoU / len(self.val_loader)))
                self.model.train()
    ...
    def validation(self):
        ...
        synchronize()
        return total_pixAcc, total_mIoU
...
if __name__ == '__main__':
    ...
    if args.distributed:
        ...
        torch.distributed.init_process_group(backend="nccl", init_method="env://")  # what??
        synchronize()  # 同步
    args.lr = args.lr * num_gpus  # why??
    ...
    runs_folder = 'runs/train/{}_{}_{}'.format(args.model, args.backbone, args.dataset)
    train_writer = SummaryWriter(runs_folder)
